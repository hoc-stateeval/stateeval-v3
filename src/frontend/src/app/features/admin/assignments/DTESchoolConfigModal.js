...
const DTESchoolConfigModal = ( { districtCode, user, setDlgOpen, dlgOpen } ) => {
  ...
  const toggleConfigureUserInRoleAtSchool = (buildingId) => {
    ...
    if (!match) {
      setBuildingRoles([...buildingRoles, {id: 0, userId: user.id, buildingId: buildingId, roleId: RoleType.DTE}])
    }
    else {
      setBuildingRoles(buildingRoles.filter(x=>x.buildingId!==buildingId));
    }
  };
  ...
  return (
    ...
    <Dialog open={dlgOpen} onClose={handleClickDlgCancel}
      maxWidth={"md"}>
      <DialogTitle>Configure District-wide Teacher Evaluator Schools</DialogTitle>
      <DialogContent>
      <Box>
          <Stack spacing={3}>
